...
import React, { useState, useEffect } from 'react';
import '../styles/JobList.css';
import { useAuth } from '../auth/AuthProvider'; // Update the import path if necessary
import axios from 'axios';

const JobList = ({ searchQuery }) => {
    const [jobPosts, setJobPosts] = useState([]);
    const { user, isLoggedIn } = useAuth(); // Use the useAuth hook to access the authentication state
